TRANSLATIONS = {

    "de": {

        "welcome": "Hallooo und herzlich willkommen in unserer Praxis 😊 Ich bin Ihr digitaler Assistent. Wie kann ich Ihnen helfen?",

        "treatments": {
            "whitening": "Zahnaufhellung",
            "implants": "Implantate",
            "braces": "Zahnspangen",
            "cleanings": "Zahnreinigung"
        },

        "great_choice": "Gute Wahl! 🦷 {treatment} gehört zu unseren häufigsten Behandlungen.\n\nMöchten Sie einen Termin buchen oder mit unserem Team sprechen?",

        "book": "Termin buchen",
        "human": "Mit Mitarbeiter sprechen",

        "which_date": "Welcher Termin passt Ihnen am besten?",
        "tomorrow": "Morgen",
        "this_week": "Diese Woche",
        "next_week": "Nächste Woche",

        "morning_afternoon": "Bevorzugen Sie einen Termin am Vormittag oder Nachmittag?",
        "morning": "Vormittag",
        "afternoon": "Nachmittag",

        "which_time": "Welche Uhrzeit passt Ihnen am besten?",
        "your_name": "Wie ist Ihr vollständiger Name?",

        "phone": "📞 Bitte geben Sie Ihre Telefonnummer an, damit wir den Termin bestätigen können.",

        "confirmation":
            "✅ Vielen Dank, {name}!<br><br>"
            "🗓️ Ihre Terminanfrage im Überblick:<br><br>"
            "• Behandlung: {treatment}<br>"
            "• Datum: {date}<br>"
            "• Uhrzeit: {time}<br><br>"
            "📞 Unser Team wird sich in Kürze bei Ihnen melden, um den Termin zu bestätigen.",

        "how_can_i_help": "Wie kann ich Ihnen helfen?",

        "main_suggestions": [
            "Termin buchen",
            "Behandlungen",
            "Versicherung",
            "Notfall"
        ],

        "treatments_reply":
            "Wir bieten Zahnaufhellung, Implantate, Zahnspangen und Zahnreinigung an. Wofür interessieren Sie sich?",

        "pricing_reply":
            "🦷 Zahnaufhellung beginnt ab 120 € und die Beratung kostet 40 €.\n\nMöchten Sie einen Termin buchen?",

        "pricing_suggestions": [
            "Termin buchen",
            "Mit Mitarbeiter sprechen"
        ],

        "insurance_reply":
            "Ja, wir akzeptieren die meisten gängigen Versicherungen.",

        "insurance_suggestions": [
            "Termin buchen",
            "Mit Rezeption sprechen"
        ],

        "booking_question":
            "🦷 Welche Behandlung möchten Sie buchen?",

        "booking_treatments": [
            "Zahnreinigung",
            "Zahnaufhellung",
            "Implantat",
            "Kontrolluntersuchung"
        ],

        "emergency_reply":
            "🚨 Bitte rufen Sie uns im Notfall direkt an. Möchten Sie unsere Telefonnummer?",

        "location_reply":
            "📍 Musterstraße 123. Mo–Fr 9:00–18:00 Uhr.",

        "human_reply":
            "Bitte hinterlassen Sie Ihren Namen und Ihre Telefonnummer, wir rufen Sie zurück.",

        "unknown_reply":
            "Ich kann Ihnen bei Terminen, Behandlungen, Versicherungen oder Notfällen helfen.",

        "error_reply":
            "Entschuldigung, das habe ich nicht verstanden. Können Sie es bitte anders formulieren?"
    },


    "en": {

        "welcome": "Hello and welcome to our practice 😊 I am your digital assistant. How can I help you today?",

        "treatments": {
            "whitening": "Whitening",
            "implants": "Implants",
            "braces": "Braces",
            "cleanings": "Cleanings"
        },

        "great_choice": "Great choice! 🦷 {treatment} is one of our most popular treatments.\n\nWould you like to book an appointment or speak with our staff?",

        "book": "Book Appointment",
        "human": "Talk to Staff",

        "which_date": "Which date works best for you?",
        "tomorrow": "Tomorrow",
        "this_week": "This Week",
        "next_week": "Next Week",

        "morning_afternoon": "Would you prefer a morning or afternoon appointment?",
        "morning": "Morning",
        "afternoon": "Afternoon",

        "which_time": "Which time suits you best?",
        "your_name": "What is your full name?",

        "phone": "📞 Please enter your phone number so we can confirm your appointment.",

        "confirmation":
            "✅ Thank you, {name}!<br><br>"
            "🗓️ Appointment request:<br><br>"
            "• Treatment: {treatment}<br>"
            "• Date: {date}<br>"
            "• Time: {time}<br><br>"
            "📞 Our team will contact you shortly to confirm your appointment.",

        "how_can_i_help": "How can I help you today?",

        "main_suggestions": [
            "Book Appointment",
            "Treatments",
            "Insurance",
            "Emergency"
        ],

        "treatments_reply":
            "We offer teeth whitening, implants, braces and professional cleanings. Which treatment are you interested in?",

        "pricing_reply":
            "🦷 Teeth whitening starts from €120 and the consultation costs €40.\n\nWould you like to book an appointment?",

        "pricing_suggestions": [
            "Book Appointment",
            "Talk to Staff"
        ],

        "insurance_reply":
            "Yes, we accept most insurance providers.",

        "insurance_suggestions": [
            "Book Appointment",
            "Talk to Reception"
        ],

        "booking_question":
            "🦷 Which treatment would you like to book?",

        "booking_treatments": [
            "Cleaning",
            "Whitening",
            "Implant",
            "Check-up"
        ],

        "emergency_reply":
            "🚨 Please call us immediately in case of an emergency. Would you like our phone number?",

        "location_reply":
            "📍 Sample Street 123. Mon–Fri 9:00–18:00.",

        "human_reply":
            "Please leave your name and phone number and we'll call you back.",

        "unknown_reply":
            "I can help you with appointments, treatments, insurance and emergencies.",

        "error_reply":
            "Sorry, I didn't understand that. Could you please rephrase your question?"
    },


    "hu": {

        "welcome": "Üdvözöljük rendelőnkben! 😊 Digitális asszisztense vagyok. Miben segíthetek?",

        "treatments": {
            "whitening": "Fogfehérítés",
            "implants": "Implantátum",
            "braces": "Fogszabályozás",
            "cleanings": "Fogtisztítás"
        },

        "great_choice": "Remek választás! 🦷 A(z) {treatment} az egyik legnépszerűbb kezelésünk.\n\nSzeretne időpontot foglalni vagy munkatársunkkal beszélni?",

        "book": "Időpontfoglalás",
        "human": "Beszélni szeretnék egy munkatárssal",

        "which_date": "Melyik időpont lenne megfelelő?",
        "tomorrow": "Holnap",
        "this_week": "Ezen a héten",
        "next_week": "Jövő héten",

        "morning_afternoon": "Délelőtti vagy délutáni időpontot szeretne?",
        "morning": "Délelőtt",
        "afternoon": "Délután",

        "which_time": "Melyik időpont megfelelő Önnek?",
        "your_name": "Mi a teljes neve?",

        "phone": "📞 Kérjük adja meg telefonszámát az időpont megerősítéséhez.",

        "confirmation":
            "✅ Köszönjük, {name}!<br><br>"
            "🗓️ Időpontigénylés:<br><br>"
            "• Kezelés: {treatment}<br>"
            "• Dátum: {date}<br>"
            "• Időpont: {time}<br><br>"
            "📞 Munkatársunk hamarosan felveszi Önnel a kapcsolatot az időpont megerősítéséhez.",

        "how_can_i_help": "Miben segíthetek?",

        "main_suggestions": [
            "Időpontfoglalás",
            "Kezelések",
            "Biztosítás",
            "Sürgősség"
        ],

        "treatments_reply":
            "Fogfehérítést, implantátumokat, fogszabályozást és professzionális fogtisztítást kínálunk. Melyik kezelés érdekli?",

        "pricing_reply":
            "🦷 A fogfehérítés ára 120 €-tól kezdődik, a konzultáció díja 40 €.\n\nSzeretne időpontot foglalni?",

        "pricing_suggestions": [
            "Időpontfoglalás",
            "Beszélni szeretnék egy munkatárssal"
        ],

        "insurance_reply":
            "Igen, a legtöbb biztosítót elfogadjuk.",

        "insurance_suggestions": [
            "Időpontfoglalás",
            "Beszélni szeretnék a recepción"
        ],

        "booking_question":
            "🦷 Milyen kezelésre szeretne időpontot foglalni?",

        "booking_treatments": [
            "Fogtisztítás",
            "Fogfehérítés",
            "Implantátum",
            "Kontrollvizsgálat"
        ],

        "emergency_reply":
            "🚨 Sürgős esetben kérjük hívjon minket telefonon. Szeretné a telefonszámunkat?",

        "location_reply":
            "📍 Minta utca 123. Hétfőtől péntekig 9:00–18:00.",

        "human_reply":
            "Kérjük adja meg nevét és telefonszámát, és hamarosan visszahívjuk.",

        "unknown_reply":
            "Segíthetek időpontfoglalásban, kezelésekkel, biztosítással vagy sürgősségi esetekkel kapcsolatban.",

        "error_reply":
            "Sajnálom, ezt nem értettem. Meg tudná fogalmazni másképp?"
    }

}